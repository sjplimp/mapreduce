...
{
  VERTEX *l = (VERTEX *) &lhs.i;
  VERTEX *r = (VERTEX *) &rhs.i;
  return l < r;
}

bool sort_by_col(const MatrixEntry &lhs, const MatrixEntry &rhs)
{
  VERTEX *l = (VERTEX *) &lhs.j;
  VERTEX *r = (VERTEX *) &rhs.j;
  return l < r;
}
...
void emit_matvec_vector(int itask, KeyValue *kv, void *ptr)
{
  ...
  INTDOUBLE value;
  static const VERTEX xvec(XVECVALUE);
...
// cout << "  kDDkDD emit_matvec_empty_terms " << row << " " << zero << endl;;
  kv->add((char *)&v, sizeof(v), (char *) &zero, sizeof(zero));
}
    ...
    if (bytes[k] == '\n') {
      if (line[0] != '%') {  // i.e., not a comment line.
        ...
        if (nzv <= 1.) {
          ...
          MatrixEntry nz(i,j, nzv);
          if (A->StorageFormat() == BY_ROW)
            kv->add((char *)&nz.i, sizeof(nz.i), (char *)&nz, sizeof(nz));
          else
            kv->add((char *)&nz.j, sizeof(nz.j), (char *)&nz, sizeof(nz));
        }
        else {
          // Valid matrix entry for pagerank problem have nzv <= 1.
          // Not general for all problems!!!!
