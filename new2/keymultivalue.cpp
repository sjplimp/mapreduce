			     ...
			     int memkalign, int memvalign, char *memfile)
{
  ...
  threelenbytes = 3*sizeof(int64_t);
...
void KeyMultiValue::add(char *key, int keybytes, char *value, int valuebytes)
{
  ...
  char *kptr = iptr + 4*sizeof(int64_t);
...
void KeyMultiValue::collapse(char *key, int keybytes, KeyValue *kv)
{
  ...
  uint64_t totalsize = 3 * sizeof(int64_t) + nkey_kv * sizeof(int);
  ...
  int64_t *intptr = (int64_t *) page;
  *(intptr++) = keybytes;
  *(intptr++) = ksize_kv + vsize_kv;
  *(intptr++) = 2*nkey_kv;
  int64_t *valuesizes = intptr;
  intptr += 2*nkey_kv;
			    ...
			    char *fpath)
{
  ...
  while (ipartition < npartition) {
	...
	if (partitions[ipartition].sp) delete partitions[ipartition].sp;
      ...
      // if set is an extended KMV pair, multiple pages will be output
...
{
  int i,keybytes,valuebytes,ibucket,ikey,ispool,dummy;
...
void KeyMultiValue::kv2kmv_extended(int iset)
{
  ...
  *((int *) page) = ncount;
  ...
  uint64_t fileoffset = pages[ipage].fileoffset + 4*sizeof(int64_t);
  fseek(fp,fileoffset,SEEK_SET);
  fwrite(&nblock,sizeof(int64_t),1,fp);
}
