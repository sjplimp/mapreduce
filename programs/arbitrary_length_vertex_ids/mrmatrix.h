...
public:
  VERTEX i;         // row index
  VERTEX j;         // column index
  double nzv;       // non-zero value.
  MatrixEntry() {};
  MatrixEntry(IDTYPE ii, IDTYPE jj, double nn) {
    i.Set(ii); j.Set(jj); nzv = nn; 
  };
  ~MatrixEntry() {};
};
...
class MRMatrix {
  public:  
    MRMatrix(MapReduce *,IDTYPE,IDTYPE,char *,int storage=0, bool storage_aware=0);
    ~MRMatrix() {Amat.clear();}

    IDTYPE NumRows() { return N; }
    IDTYPE NumCols() { return M; }
    void AddNonzero(VERTEX i, VERTEX j, double nzv) {
