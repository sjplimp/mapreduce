// ping half of pingpong test between 2 procs in ZMQ
// should launch pong first in ZMQ_PAIR mode so waits on 1st recv

// build as:
// g++4 ping.cpp -lzmq -o ping
...
int main(int narg, char **args)
{
  ...
  // ping is 1/2 of symmetric pair
  void *partner = zmq_socket(context,ZMQ_PAIR);
  // ping is client in request/reply
  //void *partner = zmq_socket(context,ZMQ_REQ);

  zmq_connect(partner,"tcp://192.168.2.108:5555");
  //zmq_connect(partner,"tcp://*:5555");
  ...
  printf("Elapsed time for %d ping of %d bytes = %g secs\n",
