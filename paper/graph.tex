...
\> \> Input: Key = $V_i$; Multivalue = edges $[V_j, w_{ij}]$ and candidate distances $d_i$ \\
...
Pre-aggregated MapReduce object $V$ = graph vertices $V_i$ with distances $d_i$ from $V_s$:  \\
\> Key = $V_i$; Value = $\infty$.\\
Pre-aggregated MapReduce object $E$ containing graph edges ($V_i, V_j$) with weights $w_{ij}$: \\
\>  Key = $V_i$; Value = $[V_j, w_{ij}]$ \\
MapReduce object $U$ to store updates to distances = empty \\
...
\> \> Find smallest candidate distance $d_{min}$ for $V_i$ \\
\> \> \> If $d_{min} \ne d_i$, $done$ = 0 \\
