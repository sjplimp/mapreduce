...
\> Compute contribution for random jumps and vertices with no outlinks. \\
\> \> Add: $V$ to $MT$. \\
\> \> Collate $MT$:  Vertex is Key. \\
\> \> Reduce $MT$:   \\
\> \> \> Input:  Key = $v_i$; MultiValue = $x_i, 0$ if $v_i \in {MT}$; MultiValue = $x_i$ if $v_i \notin {MT}$. \\
\> \> \> Compute:  Contributions; store in data pointer. \\
\> \> \> Output:  If ${nvalues} = 2$, Key = $v_i$; Value = 0. \\
\> \> MPI\_Allreduce with MPI\_SUM to compute global contributions. \\
\> Compute $y = A^T x$. 
...
\> 1 Reduce: \> Determine winner/loser vertex of each edge \\
       \> \> \> input Key = $E_{ij}$, Multivalue = NULLS or LOSER \\
       \> \> \> if any value is LOSER, emit nothing \\
...
\> 3 Reduce: \> Find LOSER vertices, delete their edges \\
       \> \> \> input Key = $V_i$, MultiValue = $V V V V ...$ \\
       \> \> \> if any $V$ is LOSER: \\
      \> \> \> \> for each $V$ in Multivalue: \\
     \> \> \> \> \> output Key = $E_{ij}$, Value = LOSER \\
	\> \> \> else: \\
      \> \> \> \> for each $V$ in Multivalue: \\
     \> \> \> \> \> output Key = $E_{ij}$, Value = NULL \\
\> 3 Collate: \> Edge as key \\
