...
\> 1 Map: \> Generate $M_{\rm remain}/P$ random edges on each processor \\
...
$M_{\rm remain} = M$ \\
while $M_{\rm remain} > 0$: \\
\> 1 Map $E_{new}$: \> Generate $M_{\rm remain}/P$ random edges on each processor \\
           \> \> output Key = $(V_i,V_j)$, Value = NULL \\
\> 1 Aggregate $E_{new}$. \\
\> 1 Add $E_{new}$ to $E_{old}$. \\
\> 1 Convert $E_{old}$ from KV to KMV. \\
\> 1 Reduce $E_{old}$: \> Remove duplicate edges \\
              \> \> input Key = $(V_i,V_j)$, MultiValue = one or more NULLs \\
              \> \> output Key = $(V_i,V_j)$, Value = NULL \\
\> $M_{\rm remain} = M - N_{kv}$
...
The improved algorithm uses MapReduce object $MR$ to store these updates.
All communication is performed using $MR$, so the total amount of communication
...
\> Aggregate $U$. \\
\> Add $U$ to $V$. Empty $U$.\\
\> Convert $V$:  Key is vertex $v_i$. \\
\> Reduce $V$:  Pick best candidate distance for each vertex. \\
...
\> \> Add $U$ to $E$.  Empty $U$. \\
\> \> Convert $E$. \\
\> \> Reduce $E$:  Generate candidate distances for neighbors of changed vertices. \\
