...
\> $M_{\rm remain} = M - N_{kv}$
 ...
 on $P$ processors, using two MapReduce objects $E_{new}$ and
 $E_{old}$.}
...
%Output:  PageRank vector $x$
 ...
 %\end{minipage}}\end{center}
%
 %\caption{\it MapReduce algorithm for PageRank vertex ranking, using
 %4 MapReduce objects $A$, $MT$, $x$, and $y$.}
%
 %\label{fig:pr}
%\end{figure}
...
$*$ Pre-aggregated MapReduce object $MT$ = indices of all-zero rows of $A$:  Key = $i$, Value = 0 \\
...
\> 2 Exit: \> if $W$ is empty, i.e. no output by Reduce 2 \\
...
Map $E$: assign random values to each vertex $V_i$ and $V_j$ in $E$ \\
...
\> \> Input: Key = $V_i$; Multivalue = edges $[V_j, w_{ij}]$ and candidate distances for $d_i$ \\
\> \> Find smallest new distance $d_{min}$ for $V_i$ \\
\> \> If $d_{min} \ne d_i$: \\
\> \> \>  $done$ = 0 \\
...
Pre-aggregated MapReduce object $V$ = graph vertices $V_i$ with distances from sourece vertex $V_s$:  \\
\> Key = $V_i$; Value = $\infty$, except Value for $V_s$ = 0 \\
Pre-aggregated MapReduce object $E$ = graph edges with weights: Key = ($V_i, V_j$), Value = $w_{ij}$ \\
MapReduce object $U$ to store updates to distances, initially empty \\
done = 0 \\
...
\> \> Input:  Key = $V_i$; Multivalue = candidate distances for $d_i$ \\
\> \> Find smallest new distance $d_{min}$ for $V_i$ \\
\> \> If $d_{min} \ne d_i$: \\
\> \> \> $done$ = 0 \\
...
\> \> \> Input:  Key = $V_i$; Multivalue = $[V_j, w_{ij}]$ and candidate distance $d_i$ if updated in previous step \\
\> \> \> If updated distance $d_i$ exists: \\
