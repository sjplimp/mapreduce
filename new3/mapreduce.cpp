						       ...
						       char *, int))
{
  ...
  uint64_t dummy;
  int memtag;
  char *scratch = mymalloc(1,dummy,memtag);
  ...
  for (int ipage = 0; ipage < npage_kmv; ipage++) {
	...
	soffset[j] = soffset[j-1] + valuesizes[j-1];
      ...
      // NOTE: what is usage of stpr and slength?
...
{
  int j,memtag,memtag_twopage;
  int nkey_kv,keybytes,valuebytes,nspool,nentry;
  uint64_t offset,dummy,dummy1,dummy2,dummy3;
  Spool **spools;
  char *ptr,*key,*value,*newpage;
  int *order;
  ...
  if (npage_kv > 1) spools = new Spool*[2*npage-1];

  // loop over pages, sort each by keys or values
  // if multiple pages, write each page out to a file as a new KV

  for (int ipage = 0; ipage < npage_kv; ipage++) {

    nkey_kv = kv->request_page(ipage,dummy1,dummy2,dummy3);

    // setup 3 arrays from 2 pages of memory
    ...
    offset = ((uint64_t) nkey_kv) * sizeof(int);
    order = (int *) twopage;
    ...
    // reorder KV pairs into new page
    // if single page, assign new page to the KV
    // if multiple pages, add page to new KV

    ptr = newpage = mymalloc(1,dummy,memtag);
    for (int i = 0; i < nkey_kv; i++) {
      j = order[i];
    ...
    if (npage_kv == 1) {
      myfree(kv->memtag);
      kv->set_page(pagesize,newpage,memtag);
    } else {
      spools[ipage] = new Spool(this,memory,error);
      spools[ipage]->set_page(pagesize,newpage);
      spools[ipage]->add(nkey_kv,ptr-newpage,newpage);
      spools[ipage]->complete();
      myfree(memtag);
    }
  }
  ...
  if (npage_kv == 1) return;

  // perform merge sort on pairs of src spools into dest spool

  myfree(kv->memtag);
  delete kv;

  int memtag1,memtag2,memtag3;
  char *mem1 = mymalloc(1,dummy,memtag1);
  char *mem2 = mymalloc(1,dummy,memtag2);
  char *mem3 = mymalloc(1,dummy,memtag3);

  int isrc = 0;
  int idest = npage_kv;

  for (int i = 0; i < npage_kv-1; i++) {
    spools[isrc]->set_page(pagesize,mem1);
    spools[isrc]->set_page(pagesize,mem2);
    spools[idest] = new Spool(this,memory,error);
    spools[idest]->set_page(pagesize,mem3);
  ...
  // convert final spool into a KV which will use page not in memhalf

  Spool *sp = spools[idest-1];
  delete [] spools;
...
void MapReduce::merge(int flag, Spool *src1, Spool *src2, Spool *dest)
{
  int result,ientry1,ientry2,nbytes1,nbytes2;
  char *str1,*str2;

  char *page1,*page2;
  int npage1 = src1->request_info(&page1);
  int npage2 = src2->request_info(&page2);

  int ipage1 = 0;
  int ipage2 = 0;
  int nentry1 = src1->request_page(ipage1);
  int nentry2 = src2->request_page(ipage2);
  ientry1 = ientry2 = 0;
  ...
  while (1) {
	...
	ipage1++;
	if (ipage1 < npage1) {
	  nentry1 = src1->request_page(ipage1);
	  ientry1 = 0;
	  ptr1 = page1;
	...
	ipage2++;
	if (ipage2 < npage2) {
	  nentry2 = src2->request_page(ipage2);
	  ientry2 = 0;
	  ptr2 = page2;
...
/* ----------------------------------------------------------------------
   create a filename with increasing counter for KV or KMV or Spool or Sort
   return filename, caller will delete it
------------------------------------------------------------------------- */

char *MapReduce::file_create(int flag)
{
  int n = strlen(fpath) + 32;
  char *fname = new char[n];
  if (flag == 0)
    sprintf(fname,"%s/mrmpi.kv.%d.%d.%d",fpath,instance_me,filecounter++,me);
  else if (flag == 1)
    sprintf(fname,"%s/mrmpi.kmv.%d.%d.%d",fpath,instance_me,filecounter++,me);
  else if (flag == 2)
    sprintf(fname,"%s/mrmpi.spl.%d.%d.%d",fpath,instance_me,filecounter++,me);
  else if (flag == 3)
    sprintf(fname,"%s/mrmpi.sort.%d.%d.%d",fpath,instance_me,filecounter++,me);
  return fname;
}
...
void MapReduce::allocate()
{
  ...
  memfull = ((uint64_t) memsize) * 1024*1024;
  memblock = (char *) memory->smalloc_align(memfull,ALIGNFILE,"MR:memblock");
  memset(memblock,0,memfull);
  allocated = 1;

  // setup data structure for partitioning memory block for KVs and KMVs

  if (npage != 4 && npage != 8) error->all("Invalid npage setting");
  pagesize = memfull/npage;
  for (int i = 0; i < npage; i++) memused[i] = 0;
}
...
char *MapReduce::mymalloc(int numpage, uint64_t &size, int &tag)
{
  if (numpage == 1) {
    tag = 0;
    while (memused[tag]) tag++;
  } else if (numpage == 2) {
    if (memused[npage-2] || memused[npage-1])
      error->all("Two-page block of memory is not available");
    tag = npage-2;
  }

  memused[tag] = 1;
  memcount[tag] = numpage;
  if (numpage == 2) memused[tag+1] = 1;

  size = numpage*pagesize;
  return &memblock[tag*pagesize];
}
...
{
  memused[tag] = 0;
  if (memcount[tag] == 2) memused[tag+1] = 0;
}
