...
class MapReduce {
 ...
 public:
  ...
  int memsize;      // # of Mbytes for block of memory MR will operate within
  int npage;        // # of pages that memsize block is partitioned into
 ...
 private:
  ...
  int filecounter;          // counter for KV and KMV filename creation

  // memory block and partitions

  char *memblock;           // memsize block of memory for entire MR
  uint64_t memfull;         // size of memory block
  uint64_t pagesize;        // size of page for KVs and KMVs
  int memused[8];           // unused/used flag for each page of block
  int memcount[8];          // count of pages allocated starting at this page
